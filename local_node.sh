...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	jq '.app_state["staking"]["params"]["bond_denom"]="apose"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	jq '.app_state["crisis"]["constant_fee"]["denom"]="apose"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	jq '.app_state["gov"]["deposit_params"]["min_deposit"][0]["denom"]="apose"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	jq '.app_state["evm"]["params"]["evm_denom"]="apose"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	jq '.app_state["inflation"]["params"]["mint_denom"]="apose"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	...
	jq -r --arg amount_to_claim "$amount_to_claim" '.app_state["bank"]["balances"] += [{"address":"evmos15cvq3ljql6utxseh0zau9m8ve2j8erz89m5wkz","coins":[{"denom":"apose", "amount":$amount_to_claim}]}]' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	...
	for KEY in "${KEYS[@]}"; do
		evmosd add-genesis-account $KEY 100000000000000000000000000apose --keyring-backend $KEYRING --home "$HOMEDIR"
	done
	...
	evmosd gentx ${KEYS[0]} 1000000000000000000000apose --keyring-backend $KEYRING --chain-id $CHAINID --home "$HOMEDIR"
...
evmosd start --pruning=nothing "$TRACE" --log_level $LOGLEVEL --gas-prices=0.0001pose --json-rpc.api eth,txpool,personal,net,debug,web3 --api.enable --home "$HOMEDIR" --keyring-backend test
