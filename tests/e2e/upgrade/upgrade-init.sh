...
# Change parameter token denominations to apose
cat "$HOME"/.evmosd/config/genesis.json | jq '.app_state["staking"]["params"]["bond_denom"]="apose"' > "$HOME"/.evmosd/config/tmp_genesis.json && mv "$HOME"/.evmosd/config/tmp_genesis.json "$HOME"/.evmosd/config/genesis.json
cat "$HOME"/.evmosd/config/genesis.json | jq '.app_state["crisis"]["constant_fee"]["denom"]="apose"' > "$HOME"/.evmosd/config/tmp_genesis.json && mv "$HOME"/.evmosd/config/tmp_genesis.json "$HOME"/.evmosd/config/genesis.json
cat "$HOME"/.evmosd/config/genesis.json | jq '.app_state["gov"]["deposit_params"]["min_deposit"][0]["denom"]="apose"' > "$HOME"/.evmosd/config/tmp_genesis.json && mv "$HOME"/.evmosd/config/tmp_genesis.json "$HOME"/.evmosd/config/genesis.json
cat "$HOME"/.evmosd/config/genesis.json | jq '.app_state["evm"]["params"]["evm_denom"]="apose"' > "$HOME"/.evmosd/config/tmp_genesis.json && mv "$HOME"/.evmosd/config/tmp_genesis.json "$HOME"/.evmosd/config/genesis.json
cat "$HOME"/.evmosd/config/genesis.json | jq '.app_state["inflation"]["params"]["mint_denom"]="apose"' > "$HOME"/.evmosd/config/tmp_genesis.json && mv "$HOME"/.evmosd/config/tmp_genesis.json "$HOME"/.evmosd/config/genesis.json

# set gov proposing && voting period
cat "$HOME"/.evmosd/config/genesis.json | jq '.app_state["gov"]["deposit_params"]["max_deposit_period"]="30s"' > "$HOME"/.evmosd/config/tmp_genesis.json && mv "$HOME"/.evmosd/config/tmp_genesis.json "$HOME"/.evmosd/config/genesis.json
cat "$HOME"/.evmosd/config/genesis.json | jq '.app_state["gov"]["voting_params"]["voting_period"]="30s"' > "$HOME"/.evmosd/config/tmp_genesis.json && mv "$HOME"/.evmosd/config/tmp_genesis.json "$HOME"/.evmosd/config/genesis.json
...
evmosd add-genesis-account $KEY 100000000000000000000000000apose --keyring-backend $KEYRING
...
evmosd gentx $KEY 1000000000000000000000apose --keyring-backend $KEYRING --chain-id "$CHAINID"
...
evmosd start --pruning=nothing "$TRACE" --log_level $LOGLEVEL --minimum-gas-prices=0.0001apose --json-rpc.api eth,txpool,personal,net,debug,web3
