...
Details of non-native currencies (ie. not `apose`) can be queried with the following CLI command:
...
Developers can query account balances of `apose` using the [`eth_getBalance`](../json-rpc/endpoints.md#ethgetbalance) JSON-RPC method in conjunction with [`curl`](https://curl.se/):
